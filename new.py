...
for the sake of simplicity each step will have a value of 1 for now , we will update it to 0.5 later on 
"""
...
class Robot:
    ...
    def reward(self):
        return self.calculate_delay() + self.calculate_energy_consumption()
...
def main():
    ...
    num_states = N_STATES
    num_episodes = 1000  # change how many episodes you want , i set a thousand as the default
    ...
    # here we can initialise with random values aswell instead of set fixed coordinates always for our 3 robots
    with open('./LOG_FIle.txt', 'a') as f:
        ...
        for episode in range(num_episodes):
            ...
            for agent, robot in enumerate(robots):
                ...
                if episode % 10 == 0:
                    if agent == 1:
                        avgrewards['1'].append(np.mean(episode_rewards[-10:]))
                    elif agent == 2:
                        avgrewards['2'].append(np.mean(episode_rewards[-10:]))
                    else:
                        avgrewards['3'].append(np.mean(episode_rewards[-10:]))
        ...
        plt.show()
    f.close()
    # Save the final rewards for each robot
